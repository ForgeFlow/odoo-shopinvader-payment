...
class PaymentServicePaypal(AbstractComponent):
    ...
    def _prepare_paypal_order(self, transaction, return_url, cancel_url, **params):
        return {
            "intent": "CAPTURE",
            "purchase_units": [
                {
    ...
    def checkout_order(self, target, payment_mode_id, return_url, cancel_url, **params):
        ...
        data = self._prepare_paypal_order(transaction, return_url, cancel_url, **params)
    ...
    def _capture_transaction(self, transaction, raise_error=False):
        ...
        response = requests.post(
            self._get_api_url(
                acquirer,
                "v2/checkout/orders/{}/capture".format(transaction.acquirer_reference),
            ),
            headers={
                "Content-Type": "application/json",
                "Authorization": token,
